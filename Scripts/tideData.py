            ...
            for root, dirnames, filenames in os.walk(mainDir):
                for filename in fnmatch.filter(filenames, '*.*'):
                    if len(filename) <= 9:
                        ...
                        y = os.path.join(root, upper_name)
                        sourcePath = y
                        if os.path.isfile(y[:-4] + '.txt'):
                            print "file exist"
                        else:
                            outPath = y[:-4] + '.txt'
...
#    sourcePath = r'C:\Users\Windows User\Desktop\Work\tide\TIDAL DATA\NOAH_orig\BALANACAN 2008-2012\DBL08.lev'
#    outPath = r'C:\Users\Windows User\Desktop\Work\tide\TIDAL DATA\NOAH_orig\BALANACAN 2008-2012\DBL08_new.txt'
#    namriaToTxt(sourcePath, outPath)
#    tidePath = os.path.dirname(outPath)
#    var = readTideTxt(tidePath, drymonths=[10])
#    guiuan = var['DBL08_new']
#    times = [t for t in guiuan.keys() if t.month == 10]
#    times.sort()
#    values = [guiuan[t] for t in times]
#    plt.plot_date(times, values, '-', label='BALANACAN Oct 2008')
#    plt.legend()
#    plt.show()
