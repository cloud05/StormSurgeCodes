...
TIDEPATH = r'C:\Users\Windows User\Desktop\Work\MindoroOutputs\Tides for Mindoro\Typhoon Reming(NovDec2006)'#r'/home/phillip/Dropbox/storm surge/Tides/Tides_November1987'
# Path where output (surge + tide) will be saved
TARGETPATH = r'C:\Users\Windows User\Desktop\Work\MindoroOutputs\reming'
csvPath = os.path.join(TARGETPATH, 'total')
# Path where HVT will be saved
outPathDir = os.path.join(csvPath, 'HVT_REMING_MINDORO')
# Typhoon name
tyname = '2006_reming'


STATIONLISTPATH = [r'F:\Actual Strength Simulation',
                   r'G:\ACTUAL STRENGTH SIMULATIONS']
...
for stationCombination in stations:
    ...
    if not os.path.exists(station):
        continue
    for storm in sorted(listdir_fullpath(station)):
