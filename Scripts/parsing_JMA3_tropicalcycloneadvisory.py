...
# Purpose:
#
# Author:      nigs-mecoteco
#
# Created:     04/02/2014
# Copyright:   (c) nigs-mecoteco 2014
...
def ensemble(tyDF, how):
    """
    Accepts a dataframe of typhoon data, with columns 'lat', 'lon', and '70radius'.
    Returns the same dataframe with modified coordinates based on kwarg 'how'.
    choices for how = ['fast', 'slow', 'right', 'left']
    """
    from mesh import Line
    from mesh import Point
    
    radius = tyDF['70radius']
    lats = tyDF['lat']
    lons = tyDF['lon']
    modifiedTyDF = tyDF.copy()
    modifiedlats = modifiedTyDF['lat']
    modifiedlons = modifiedTyDF['lon']
    
    
    for i in xrange(1, len(df)):
        if radius[i] > 0.0:
            p1 = Point((lons[i - 1], lats[i - 1]))
            p2 = Point((lons[i], lats[i]))
            line = Line(p1, p2)
            r = radius[i] / 60.0
            if line.hlength > 0 and line.vlength > 0:
                theta = math.atan(line.vlength / line.hlength)
            elif line.hlength < 0 and line.vlength > 0:
                theta = math.pi - math.atan(abs(line.vlength / line.hlength))
            elif line.hlength < 0 and line.vlength < 0:
                theta = math.pi + math.atan(abs(line.vlength / line.hlength))
            elif line.hlength > 0 and line.vlength < 0:
                theta = 2 * math.pi - math.atan(abs(line.vlength / line.hlength))
            elif line.hlength == 0 and line.vlength > 0:
                theta = math.pi / 2
            elif line.hlength == 0 and line.vlength < 0:
                theta = 3 * math.pi / 2
            elif line.vlength == 0 and line.hlength > 0:
                theta = 0.0
            elif line.vlength == 0 and line.hlength < 0:
                theta = math.pi
            else:
                pass #Do something for lines with same endpoints, (degenerate)
            if how == 'fast':
                deltaLon = math.cos(theta) * r
                deltaLat = math.sin(theta) * r
            elif how == 'slow':
                deltaLon = math.cos(theta) * r * -1
                deltaLat = math.sin(theta) * r * -1
            elif how == 'right':
                deltaLon = math.sin(theta) * r
                deltaLat = math.cos(theta) * r * -1
            elif how == 'left':
                deltaLon = math.sin(theta) * r * -1
                deltaLat = math.cos(theta) * r
            else:
                raise Exception("choices for how = ['fast', 'slow', 'right', 'left']")
            modifiedlons[i] = lons[i] + deltaLon
            modifiedlats[i] = lats[i] + deltaLat
            modifiedTyDF['lon'] = modifiedlons
            modifiedTyDF['lat'] = modifiedlats
        else:
            continue
    return np.round(modifiedTyDF, 3)
    
    
def writeJMA1(df, outdata, path, year, numstr, how, append='0000'):
    Pcenter_sorted = df['Pcenter'].values
    Mxwd_sorted = df['Mxwd'].values
    lon_sorted = df['lon'].values
    lat_sorted = df['lat'].values
    
    outpath = os.path.join(path, 'bst' + str(year) + '.txt')
    f = open(outpath, how) 
    f.write('66666 ' + str(year)[-2:] + numstr + '   ' + str(len(outdata['Dates'])) +
            '      ' + str(year)[-2:] + numstr + ' 0 6 ' +
            outdata['Name'].split()[2] + append + '\n'  )
...
#    path = r'/home/phillip/Dropbox/scripts/JMAparse'
#    path = r'C:\Users\user\Dropbox\scripts\JMAparse'
    path = r'C:\Users\user\Dropbox\Forecast\Pablo'
    ...
    dtnow = datetime.datetime.now() - datetime.timedelta(hours=8)# - datetime.timedelta(days=9)
...
#    data['Time'] = datetime.datetime(dtnow.year, dtnow.month, int(advisorytime[0:2]), int(advisorytime[2:4]), int(advisorytime[4:6]))
    data['Time'] = datetime.datetime(2012, 12, int(advisorytime[0:2]), int(advisorytime[2:4]), int(advisorytime[4:6]))
    data['Type'] = lines[1]
    print lines
    ...
    outdata['70radius'] = [0 for date in outdata['Dates']]
...
#    # if data for date is not yet in the list, include it in the list
#    if date_to_string(data['Time']) not in outdata['Dates']:
#        # append Analysis data
#        outdata['Dates'].append(date_to_string(data['Time']))
#        outdata['Pcenter'].append(float(data['Analysis']['PRES'].strip('HPA')))
#        outdata['Mxwd'].append(float(data['Analysis']['MXWD'].strip('KT')))
...
#        outdata['Coef'].append(0.70)
#        outdata['Pfar'].append(1012.00)
#    # if date already contains data, replace data with latest
#    elif date_to_string(data['Time']) in outdata['Dates']:
#        idx_to_update = outdata['Dates'].index(date_to_string(data['Time']))
#        outdata['Pcenter'][idx_to_update] = float(data['Analysis']['PRES'].strip('HPA'))
#        outdata['Mxwd'][idx_to_update] = float(data['Analysis']['MXWD'].strip('KT'))
#        outdata['lon'][idx_to_update] = data['Analysis']['Longitude']
#        outdata['lat'][idx_to_update] = data['Analysis']['Latitude']
#        outdata['r0'][idx_to_update] = 60
#        outdata['Coef'][idx_to_update] = 0.70
#        outdata['Pfar'][idx_to_update] = 1012.00
#
#    # append Forecast data
#    for ftime in data['Forecast Times']:
#        ftime_dict = data['Forecast'][ftime]
#        if date_to_string(ftime_dict['Time']) not in outdata['Dates']:
#            outdata['Dates'].append(date_to_string(ftime_dict['Time']))
#            outdata['Pcenter'].append(float(ftime_dict['PRES'].strip('HPA')))
#            outdata['Mxwd'].append(float(ftime_dict['MXWD'].strip('KT')))
#            outdata['lat'].append(ftime_dict['Latitude'])
...
#            outdata['Coef'].append(0.70)
#            outdata['Pfar'].append(1012.00)
#        elif date_to_string(ftime_dict['Time']) in outdata['Dates']:
#            idx_to_update = outdata['Dates'].index(date_to_string(ftime_dict['Time']))
#            outdata['Pcenter'][idx_to_update] = float(ftime_dict['PRES'].strip('HPA'))
#            outdata['Mxwd'][idx_to_update] = float(ftime_dict['MXWD'].strip('KT'))
#            outdata['lon'][idx_to_update] = ftime_dict['Longitude']
#            outdata['lat'][idx_to_update] = ftime_dict['Latitude']
#            outdata['r0'][idx_to_update] = 60
#            outdata['Coef'][idx_to_update] = 0.70
#            outdata['Pfar'][idx_to_update] = 1012.00
    ...
    dateObjects = [string_to_date(date) for date in Dates_sorted]
    ...
    # update value lists
    dtrng = [date for date in pd.to_datetime(rng)]
    strrng = [date_to_string(date) for date in dtrng]
    ...
    writeJMA1(df, outdata, path, 2012, '01', how='w')
    fastdf = ensemble(df, how='fast')
    fastx = fastdf['lon'].values
    fasty = fastdf['lat'].values
    writeJMA1(fastdf, outdata, path, 2012, '02', how='a', append='0001')
    slowdf = ensemble(df, how='slow')
    slowx = slowdf['lon'].values
    slowy = slowdf['lat'].values
    writeJMA1(slowdf, outdata, path, 2012, '03', how='a', append='0002')
    leftdf = ensemble(df, how='left')
    leftx = leftdf['lon'].values
    lefty = leftdf['lat'].values
    writeJMA1(leftdf, outdata, path, 2012, '04', how='a', append='0003')
    rightdf = ensemble(df, how='right')
    rightx = rightdf['lon'].values
    righty = rightdf['lat'].values
    writeJMA1(rightdf, outdata, path, 2012, '05', how='a', append='0004')
    
    plt.plot(lon_sorted, lat_sorted, 'o-', label='Track', linewidth=2)
    plt.plot(fastx, fasty, 'o-', label='Fast', linewidth=2)
    plt.plot(slowx, slowy, 'o-', label='Slow', linewidth=2)
    plt.plot(leftx, lefty, 'o-', label='Left', linewidth=2)
    plt.plot(rightx, righty, 'o-', label='Right', linewidth=2)
    plt.legend()
    
    for i in xrange(0, len(df)):
        c = plt.Circle((df['lon'][i], df['lat'][i]), df['70radius'][i] / 60, fill=False, color='k')
        print df['lon'][i], df['lat'][i], df['70radius'][i] 
